...
class SrcMariabackupTests(unittest.TestCase):
    ...
    def _wait_for_server(self, container):
        ...
        for i in range(30):
            if subprocess.run(["podman", "exec", "-i", container, "mariadb", "-uroot", "-ptest", "-e", "SELECT VERSION()"], capture_output=True).returncode == 0:
                break
            time.sleep(1)
    ...
    def _run_server(self, version):
        return subprocess.check_output(["podman", "run", "--rm",
    ...
    def tearDown(self):
        subprocess.check_call(["podman", "unshare", "rm", "-rf", self.tmpdir])
    ...
    def test_mariabackup_source(self):
        ...
        try:
            ...
            subprocess.run(["podman", "exec", "-i", container, "mariadb", "-uroot", "-ptest"], check=True, input=b"CREATE DATABASE ubkptest")
            subprocess.run(["podman", "exec", "-i", container, "mariadb", "-uroot", "-ptest", "ubkptest"], check=True, input=b"CREATE TABLE test(a INT); INSERT INTO test VALUES (1);")
            subprocess.check_call([uback, "backup", "-n", source, dest])
            time.sleep(0.01)

            subprocess.run(["podman", "exec", "-i", container, "mariadb", "-uroot", "-ptest", "ubkptest"], check=True, input=b"INSERT INTO test VALUES (2), (3);")
            subprocess.check_call([uback, "backup", "-n", source, dest])
            time.sleep(0.01)

            subprocess.run(["podman", "exec", "-i", container, "mariadb", "-uroot", "-ptest", "ubkptest"], check=True, input=b"UPDATE test SET a=4 WHERE a=1; DELETE FROM test WHERE a=2;")
            subprocess.check_call([uback, "backup", "-n", source, dest])
            time.sleep(0.01)
            ...
            if shutil.which("mariadb") is not None and not os.getenv("MARIADB_TESTS_FORCE_LOCAL") == "1":
                ...
                subprocess.check_call([uback, "restore", "-d", f"{self.tmpdir}/restore", "-o", "use-podman=false", dest])
                restore_path = os.listdir(f"{self.tmpdir}/restore")[0]
