...
TEST_TMPDIR="$BATS_RUN_TMPDIR/src-tar"

@test "tar source" {
	mkdir -p "$TEST_TMPDIR"

	$UBACK key gen "$TEST_TMPDIR/backup.key" "$TEST_TMPDIR/backup.pub"
	source=type=tar,path="$TEST_TMPDIR/source",key-file="$TEST_TMPDIR/backup.pub",state-file="$TEST_TMPDIR/state.json",snapshots-path="$TEST_TMPDIR/snapshots",full-interval=weekly,@command=tar,@command=--exclude=./c,@command=--exclude=./d
